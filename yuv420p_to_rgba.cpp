// yuv420p_to_rgba.cpp  (YUV420P -> RGBA, pass owns output)
...
Yuv420pToRgbaPass::Yuv420pToRgbaPass(Engine2D* engine,
      ...
      inputDescriptorType_(inputDescriptorType)
{
    if (!engine_ || engine_->logicalDevice == VK_NULL_HANDLE)
        throw std::runtime_error("Yuv420pToRgbaPass: invalid engine");

    if (framesInFlight_ == 0 || framesInFlight_ > 16)
        throw std::runtime_error("Yuv420pToRgbaPass: framesInFlight must be 1..16");

    if (width_ <= 0 || height_ <= 0)
        throw std::runtime_error("Yuv420pToRgbaPass: invalid dimensions");

    // Your current GLSL uses sampler2D for yTex/uTex/vTex:
    ...
    if (!isSampledInputType(inputDescriptorType_))
        throw std::runtime_error("Yuv420pToRgbaPass: shader uses sampler2D, so inputDescriptorType must be VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER");
}

Yuv420pToRgbaPass::~Yuv420pToRgbaPass()
{
    if (!engine_ || engine_->logicalDevice == VK_NULL_HANDLE)
        return;
...
void Yuv420pToRgbaPass::initialize()
{
    ...
    if (renderDebugEnabled())
        std::cout << "[Yuv420pToRgbaPass] initialized framesInFlight=" << framesInFlight_
                  << " size=" << width_ << "x" << height_ << std::endl;
}

void Yuv420pToRgbaPass::resize(int width, int height)
{
...
void Yuv420pToRgbaPass::setInputYUV420P(VkImageView yView,
                                        VkImageView uView,
                                        VkImageView vView,
                                        VkSampler ySampler,
                                        VkSampler uSampler,
                                        VkSampler vSampler)
{
    ...
    if (yView_ != VK_NULL_HANDLE && uView_ != VK_NULL_HANDLE && vView_ != VK_NULL_HANDLE)
    {
        if (ySampler_ == VK_NULL_HANDLE || uSampler_ == VK_NULL_HANDLE || vSampler_ == VK_NULL_HANDLE)
            throw std::runtime_error("Yuv420pToRgbaPass: sampler2D inputs require non-null ySampler + uSampler + vSampler");
    }
...
void Yuv420pToRgbaPass::dispatch(VkCommandBuffer cmd, uint32_t frameIndex)
{
    ...
    if (yView_ == VK_NULL_HANDLE || uView_ == VK_NULL_HANDLE || vView_ == VK_NULL_HANDLE ||
        ySampler_ == VK_NULL_HANDLE || uSampler_ == VK_NULL_HANDLE || vSampler_ == VK_NULL_HANDLE)
        return;
    ...
    vkCmdPushConstants(cmd,
                       ...
                       0,
                       sizeof(yuv420pToBGRPushConstants),
                       &pushConstants);
...
PresentInput Yuv420pToRgbaPass::output(uint32_t frameIndex) const
{
    const uint32_t fi = frameIndex % framesInFlight_;
...
VkImageView Yuv420pToRgbaPass::outputView(uint32_t frameIndex) const
{
    return outViews_[frameIndex % framesInFlight_];
}

VkImage Yuv420pToRgbaPass::outputImage(uint32_t frameIndex) const
{
    return outImages_[frameIndex % framesInFlight_];
}

void Yuv420pToRgbaPass::createPipeline_()
{
    // bindings 0,1,2,3 must match SPIR-V compiled from the GLSL:
    // 0 = yTex (sampler2D)  -> COMBINED_IMAGE_SAMPLER
    // 1 = uTex (sampler2D)  -> COMBINED_IMAGE_SAMPLER
    // 2 = vTex (sampler2D)  -> COMBINED_IMAGE_SAMPLER
    // 3 = rgbaOutput (storage image)
    ...
    if (vkCreateDescriptorSetLayout(engine_->logicalDevice, &dsl, nullptr, &descriptorSetLayout_) != VK_SUCCESS)
        throw std::runtime_error("Yuv420pToRgbaPass: failed to create descriptor set layout");

    VkPushConstantRange push{};
    push.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    push.offset = 0;
    push.size = sizeof(yuv420pToBGRPushConstants);
    ...
    if (vkCreatePipelineLayout(engine_->logicalDevice, &pli, nullptr, &pipelineLayout_) != VK_SUCCESS)
        throw std::runtime_error("Yuv420pToRgbaPass: failed to create pipeline layout");

    // Make sure this SPIR-V is compiled from the sampler2D version of the shader.
    auto shaderCode = readSPIRVFile("shaders/yuv420p_to_rgba.spv");
    ...
    if (vkCreateComputePipelines(engine_->logicalDevice, VK_NULL_HANDLE, 1, &cpi, nullptr, &pipeline_) != VK_SUCCESS)
    {
        ...
        throw std::runtime_error("Yuv420pToRgbaPass: failed to create compute pipeline");
    }

    vkDestroyShaderModule(engine_->logicalDevice, shaderModule, nullptr);
}

void Yuv420pToRgbaPass::destroyPipeline_()
{
    if (!engine_ || engine_->logicalDevice == VK_NULL_HANDLE)
        return;
...
void Yuv420pToRgbaPass::createOutputs_()
{
    destroyOutputs_();
    ...
    for (uint32_t i = 0; i < framesInFlight_; ++i)
    {
        ...
        if (vkCreateImage(engine_->logicalDevice, &ii, nullptr, &outImages_[i]) != VK_SUCCESS)
            throw std::runtime_error("Yuv420pToRgbaPass: failed to create output image");
        ...
        if (vkAllocateMemory(engine_->logicalDevice, &ai, nullptr, &outMem_[i]) != VK_SUCCESS)
            throw std::runtime_error("Yuv420pToRgbaPass: failed to allocate output image memory");
        ...
        if (vkCreateImageView(engine_->logicalDevice, &vi, nullptr, &outViews_[i]) != VK_SUCCESS)
            throw std::runtime_error("Yuv420pToRgbaPass: failed to create output image view");
    }
...
void Yuv420pToRgbaPass::destroyOutputs_()
{
    if (!engine_ || engine_->logicalDevice == VK_NULL_HANDLE)
        return;
...
void Yuv420pToRgbaPass::createDescriptors_()
{
    destroyDescriptors_();
    ...
    // yTex + uTex + vTex are COMBINED_IMAGE_SAMPLER (3 per set)
    sizes[0].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    sizes[0].descriptorCount = framesInFlight_ * 3;
    ...
    if (vkCreateDescriptorPool(engine_->logicalDevice, &pi, nullptr, &descriptorPool_) != VK_SUCCESS)
        throw std::runtime_error("Yuv420pToRgbaPass: failed to create descriptor pool");
    ...
    if (vkAllocateDescriptorSets(engine_->logicalDevice, &ai, descriptorSets_.data()) != VK_SUCCESS)
        throw std::runtime_error("Yuv420pToRgbaPass: failed to allocate descriptor sets");

    rebuildDescriptorSets_();
}

void Yuv420pToRgbaPass::destroyDescriptors_()
{
    if (!engine_ || engine_->logicalDevice == VK_NULL_HANDLE)
        return;
...
void Yuv420pToRgbaPass::rebuildDescriptorSets_()
{
    if (!engine_ || descriptorPool_ == VK_NULL_HANDLE)
        return;

    // If inputs aren't set yet, skip; we'll rebuild when setInputYUV420P is called.
    if (yView_ == VK_NULL_HANDLE || uView_ == VK_NULL_HANDLE || vView_ == VK_NULL_HANDLE ||
        ySampler_ == VK_NULL_HANDLE || uSampler_ == VK_NULL_HANDLE || vSampler_ == VK_NULL_HANDLE)
        return;
    ...
    for (uint32_t i = 0; i < framesInFlight_; ++i)
    {
        ...
        // Binding 1: uTex (combined sampler)
        VkDescriptorImageInfo uInfo{};
        uInfo.imageView = uView_;
        uInfo.sampler = uSampler_;
        uInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        ...
        // Binding 2: vTex (combined sampler)
        VkDescriptorImageInfo vInfo{};
        vInfo.imageView = vView_;
        vInfo.sampler = vSampler_;
        vInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        ...
        // Binding 3: rgbaOutput (storage image)
...
void Yuv420pToRgbaPass::createOutputSampler_()
{
    destroyOutputSampler_();
    ...
    if (vkCreateSampler(engine_->logicalDevice, &si, nullptr, &outputSampler_) != VK_SUCCESS)
        throw std::runtime_error("Yuv420pToRgbaPass: failed to create output sampler");
}

void Yuv420pToRgbaPass::destroyOutputSampler_()
{
    if (!engine_ || engine_->logicalDevice == VK_NULL_HANDLE)
        return;
