...
std::vector<SubtitleLine> buildLines(const std::vector<SubtitleSegment>& segments)
{
    ...
    std::sort(lines.begin(), lines.end(), [](const SubtitleLine& a, const SubtitleLine& b) {
...
bool initializeSubtitleOverlayCompute(Engine2D* engine, SubtitleOverlayCompute& comp)
{
    ...
    auto shaderCode = readSPIRVFile("shaders/subtitle.comp.spv");
...
bool Subtitleload(const std::filesystem::path& path)
{
...
std::vector<const SubtitleLine*> SubtitleactiveLines(double currentTime, size_t maxLines) const
{
                           ...
                           bool enableBackground)
{
    ...
    const std::vector<const SubtitleLine*> candidateLines = overlay.activeLines(currentTime, maxLines);
    ...
    std::vector<const SubtitleLine*> lines;
    lines.reserve(candidateLines.size());
    std::string lastText;
    for (const SubtitleLine* line : candidateLines)
    {
        if (!line)
        {
            continue;
